...
func (p *Provider) ImportDataForDeal(ctx context.Context, propCid cid.Cid, data io.Reader, path string) error {
	...
	ifUsePath := os.Getenv("MARKET_SHARE_OFFLINE_CAR")

	var tempPath filestore.Path
	if ifUsePath == "true" {
		tempPath = filestore.Path(path)
	} else {
		// copy to temp file
		...
		tempPath = tempfi.Path()

		enableVerify := os.Getenv("MARKET_ENABLE_OFFLINE_VERIFY")
	...
	return p.deals.Send(propCid, storagemarket.ProviderEventVerifiedData, tempPath, filestore.Path(""))
}
